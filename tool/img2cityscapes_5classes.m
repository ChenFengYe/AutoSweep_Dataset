cube_labelpath = 'D:\Desktop\ObjectSnapData\Cube_Cylinder_11658\seg\*.png';
instpath = 'D:\Desktop\ObjectSnapData\Cube_Cylinder_11658\instanceIds_5classes\';
class_num = 5;
    ...
    for r = 1:size(img,1)
        for c = 1:size(img,2)
            if get_Categorie(img(r,c,:))==class_i
                ...
                color = img(r,c,:);
                if color(3) == 150
                    add_inst = add_inst + 100;
                end
                %
                
                seg(r,c) = (class_i*1000)+uint16(img(r,c,1)/10+add_inst);
